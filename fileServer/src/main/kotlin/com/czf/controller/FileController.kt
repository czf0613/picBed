...
class FileController {
    ...
    fun delete(@PathVariable("fileId")fileId: Int): ResponseEntity<String> = runBlocking {
        val token = httpServletRequest.getHeader("token")
        if(!tokenService.checkToken(token))
            return@runBlocking ResponseEntity("非法用户", HttpStatus.FORBIDDEN)

        val filePath = fileDAO.findFilePathById(fileId)
        fileService.deleteFileAt(filePath)
        ResponseEntity("删除成功", HttpStatus.OK)
    }
}