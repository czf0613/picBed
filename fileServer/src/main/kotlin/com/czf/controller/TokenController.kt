...
class TokenController {
    ...
    fun getAllToken(@PathVariable("userId")userId:Int):ResponseEntity<List<Token>> {
        if(!userService.checkStatus(httpServletRequest.getHeader("token")))
            return ResponseEntity(HttpStatus.FORBIDDEN)

        return ResponseEntity(tokenDAO.findAllByOwner(userId), HttpStatus.OK)
    }

    @RequestMapping(value = ["/{userId}/generate"], method = [RequestMethod.POST])
    fun generate(@PathVariable("userId")userId:Int, @RequestParam("label",required = false, defaultValue = "新建token")label:String):ResponseEntity<Token> {
        if(!userService.checkStatus(httpServletRequest.getHeader("token")))
            return ResponseEntity(HttpStatus.FORBIDDEN)
